...
**Efficiency:** By focusing only on the most important parameters, models can train faster and use less computing power this is especially valuable when working with massive models like large language models (LLMs).​
...
After Injection: Small LoRA matrices (A, B) are added around the linear projections.

Only the LoRA weights are trained, while the base GPT weights stay frozen.
